...
void printGrades(int player) // 플레이어의 성적 출력하는 함수 
{
	...
	else
	{
    	...
    	for (i=0;i<smmdb_len(LISTNO_OFFSET_GRADE + player);i++)
    	{
        	...
        	printf("%s : (credit: %d): %s\n", smmObj_getNodeName(gradePtr), smmObj_getNodeGrade(gradePtr), smmObj_getNodeGradeType(gradePtr));
    	}
	}
}
...
void printPlayerStatus(void) // 플레이어의 상태 출력하는 함수 
{
    ...
    {
    	boardPtr = smmb_getData(LISTNO_NODE, cur_[player[i]. position);
    	type = smmObj_getNodeType(boardPtr);
    	if (type == SMNODE_TYPE_LABORATORY && cur_player[i].temp == 1)
		{
...
void generatePlayers(int n, int initEnergy) //generate a new player
{
    ...
    for (i=0;i<n;i++)
    {
        ...
        cur_player[i].flag_graduate = 0;//set energy
